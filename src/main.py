...
def monitor_service(item, debug):
    ...
    while True:
        if check_type == 'http':
            ...
            logger.debug(f"Checking [{name}] ({check_type.upper()}/SSL Check={str(check_ssl)}) at {service_endpoint} for {monitoring_url}")
            ...
            logger.debug(f"Checking [{name}] ({check_type.upper()}) at {service_endpoint}:{str(tcp_port)} for {monitoring_url}")
            res = check_serviceTCP(service_endpoint, tcp_port, tcp_timeout)
            tested_endpoint = 'tcp://' + service_endpoint + ':' + str(tcp_port)

        if res:
            logger.info(f"Service [{name}] UP at {tested_endpoint}")
            post_healthchecksio_status(monitoring_url)
        else:
            logger.error(f"Service [{name}] DOWN at {tested_endpoint}")
            post_healthchecksio_status(monitoring_url + "/fail")

        logger.debug(f"Sleeping {polling_timer} seconds before next check for [{name}]...")
        logger.setLevel(current_config.get('config.logs.log_level').upper())
...
def main():
    if current_config.get('config.logs.log_level').upper() == "DEBUG":
        ...
        logger.debug('{:#^80s}'.format(' BEGINNING '))
        logger.debug('\n\n' + yaml.dump(current_config.config, default_flow_style=False))
        logger.debug('{:#^80s}'.format(' END '))
